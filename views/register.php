...
<head>
    ...
    <style>
        ...
        body {
            ...
            /* THAY ĐỔI LỚN NHẤT: SỬ DỤNG LINEAR GRADIENT CHO MÀU NỀN */
            background-image: linear-gradient(to right, #e3f2ff, #fff0f5); 
            /* Bắt đầu từ Xanh nhạt (#e3f2ff) ở bên trái và chuyển dần sang Hồng nhạt (#fff0f5) ở bên phải */
        ...
        .register-card { 
            max-width: 400px;
            width: 100%;
            /* Tăng độ nổi bật của box trên nền gradient */
            box-shadow: 0 10px 30px rgba(0, 0, 0, 0.2); 
            background-color: white; /* Đảm bảo box đăng ký có nền trắng */
            border-radius: 15px; 
        }
        .btn-custom-blue {
...
<body>
    ...
    <div class="card register-card p-5"> 
        <div class="card-body">
            ...
            <form id="registerForm" action="<?= BASE_URL ?>?action=register" method="post" novalidate>
    <div class="mb-3">
    <input type="email" id="email" name="email" class="form-control rounded-pill py-2" placeholder="Email" value="<?= htmlspecialchars($old['email'] ?? '') ?>">
    <p id="emailError" class="text-danger small mt-1"><?= $errors['email'] ?? '' ?></p>
</div>

<div class="mb-3">
    <input type="text" id="name" name="name" class="form-control rounded-pill py-2" placeholder="Họ và Tên" value="<?= htmlspecialchars($old['name'] ?? '') ?>">
    <p id="nameError" class="text-danger small mt-1"><?= $errors['name'] ?? '' ?></p>
</div>

<div class="mb-3">
    <input type="tel" id="phone" name="phone" class="form-control rounded-pill py-2" placeholder="Số điện thoại" value="<?= htmlspecialchars($old['phone'] ?? '') ?>">
...
<div class="mb-4">
    <input type="password" id="repass" name="repass" class="form-control rounded-pill py-2" placeholder="Xác nhận Mật khẩu">
    ...
    <p class="text-center mt-4 small">
        Đã có tài khoản? 
        <a href="<?= BASE_URL . '?action=login' ?>" class="text-custom-info text-decoration-none">Đăng nhập ngay</a>
    </p>
</form>
...
document.getElementById('registerForm').addEventListener('submit', function(e){
    ...
    if(!valid) e.preventDefault(); // Ngăn form submit nếu có lỗi
});
</script> -->
